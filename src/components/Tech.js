...
export default class Tech extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      const parsedData = await fetchNews({ domains: 'techcrunch.com,thenextweb.com', pageSize: 20 });
  ...
  previousPage = async () => {
    ...
    try {
      const parsedData = await fetchNews({ domains: 'techcrunch.com,thenextweb.com', pageSize: 20, page: this.state.page - 1 });
      console.log(parsedData);
      if (parsedData.status === "ok") {
